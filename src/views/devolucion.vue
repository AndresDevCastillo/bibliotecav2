    ...
    <v-container
        ...
        <v-card>
            <v-card-title class="pb-0">
                <h3 class="headline mb-0">Devolución de préstamos</h3>
            </v-card-title>
            <v-card-text>
                                    ...
                                    <v-row no-gutters>
                                        <v-col cols="12" class="pr-0 pb-0">
                                            <v-autocomplete
                                                ...
                                                :items="usuarios"
                                                :item-text="(usuario) => { return `${usuario.cedula} - ${usuario.nombre} ${usuario.apellido}` }"
                                                item-value="cedula"
                                                label="Cédula"
                                                :rules="campoRules"
                                                filled
                        ...
                        <template v-slot:item.actions="{ index }">
                            <v-tooltip top color="info">
                                <template v-slot:activator="{ on, attrs }">
                                    <v-icon
                                        color="var(--c-orange)" v-bind="attrs" v-on="on" @click="verDetalles(index)">
                                        mdi mdi-eye
                                    </v-icon>
                                </template>
        ...
        <v-row justify="space-around">
            <v-col cols="auto">
                <v-dialog
                    ...
                    max-width="900" v-model="dialogDetalle">
                    <v-card>
...
export default {
    ...
    data: () => ({
        ...
        headersPrestamo: [{ text: 'Código', value: 'id' },
        ...
        itemsPrestamo: [],
        prestamosTabla: [],
        headersDetalle: [{ text: 'Tipo equipo', value: 'tipo_equipo', sortable: false },
    ...
    methods: {
        verDetalles(index) {
            if (index != null) {
                ...
                this.itemsDetalle = this.itemsPrestamo[index].detalle.map(detalle => {
                    ...
                    this.estadosSeleccionados.push({ id: detalle.equipo.estado_equipo.id, estado: detalle.equipo.estado_equipo.estado });
                    return {
        ...
        async buscarPrestamos() {
            if (this.$refs.form.validate()) {
                await axios.get(`${this.rutaBackend}/prestamo/usuario/${this.paquete.cedula}`).then(response => {
                    ...
                    this.itemsPrestamo.forEach(prestamo => {
                        ...
                        const f1 = new Date(Date.UTC(parseInt(p1[0]), parseInt(p1[1]) - 1, parseInt(m1[0]), m1[1].slice(0, 2), 0, 0));
                        //Fecha de fin formateada
                        const f2 = new Date(Date.UTC(parseInt(p2[0]), parseInt(p2[1]) - 1, parseInt(m2[0]), m2[1].slice(0, 2), 0, 0));
                        this.prestamosTabla.push({ id: prestamo.id, fecha_inicio: `${f1.getFullYear()}-${(f1.getMonth() < 10 ? '0' : '') + f1.getMonth()}-${(f1.getDate() < 10 ? '0' : '') + f1.getDate()} ${(f1.getHours() < 10 ? '0' : '') + f1.getHours()}:00`, fecha_fin: `${f2.getFullYear()}-${(f2.getMonth() < 10 ? '0' : '') + f2.getMonth()}-${(f2.getDate() < 10 ? '0' : '') + f2.getDate()} ${(f2.getHours() < 10 ? '0' : '') + f2.getHours()}:00` });
                    });
                }).catch(error => {
                    console.log(error);
                });
        ...
        async entregar() {
            if (this.validarEstados()) {
                const paquete = {
                    ...
                    equipos: this.itemsPrestamo[this.indexSelecionado].detalle.map((detalle, index) => {
                        return {
                            id_equipo: detalle.equipo.id,
                            estado_equipo: this.estadosSeleccionados[index].id,
                            observacion: this.observaciones[index]
                        }
                    }),
                };
                await axios.post(`${this.rutaBackend}/prestamo/entregar`, paquete).then(response => {
        ...
        validarEstados() {
            for (let i = 0; i < this.estadosSeleccionados.length; i++) {
                if (this.estadosSeleccionados[i].estado.toLowerCase() != 'bueno' && /^[ ]*$/gm.test(this.observaciones[i])) {
                    this.paqueteMsj.title = 'Entrega de equipos';
                    this.paqueteMsj.body = 'Los equipos con estado MALO deben llevar su observación';
